...
const SummaryStep = ({ form, shipmentTypes, paymentMethods }) => {
  const calculateCostBreakdown = () => {
    const shipmentType = shipmentTypes.find((t) => t.id === form.shipmentType);
    const weight = parseFloat(form.package.weight) || 0;
    const baseFee = shipmentType.base;
    const weightFee = weight * shipmentType.kg;
        ...
        <div className="space-y-2 text-sm">
          <p>
            ...
            <span className="font-semibold">
              {shipmentTypes.find((s) => s.id === form.shipmentType)?.label}
            </span>
          </p>
          <p>
            <span className="text-gray-600">Cân nặng:</span>{' '}
            <span className="font-semibold">{form.package.weight}kg</span>
          </p>
          <p>
            <span className="text-gray-600">Kích thước:</span>{' '}
            <span className="font-semibold">
              {form.package.length || '?'} × {form.package.width || '?'} × {form.package.height || '?'} cm
            </span>
          </p>
          <p>
            ...
            return (
              <div
                ...
                className={`p-4 border-2 rounded-lg text-center ${
                  form.payment === m.id ? 'border-blue-600 bg-blue-50' : 'border-gray-200'
                }`}
              >
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
        ...
        <div className="space-y-3 text-sm mb-4">
          <div className="flex justify-between">
            ...
            <span className="font-semibold">{costBreakdown.baseFee.toLocaleString('vi-VN')}₫</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">
              Phí cân nặng ({costBreakdown.weight}kg × {costBreakdown.kgRate}₫):
            </span>
            <span className="font-semibold">{costBreakdown.weightFee.toLocaleString('vi-VN')}₫</span>
          </div>
          {costBreakdown.fragileFee > 0 && (
            <div className="flex justify-between text-red-600">
              <span>Phí hàng dễ vỡ:</span>
              <span className="font-semibold">{costBreakdown.fragileFee.toLocaleString('vi-VN')}₫</span>
            </div>
          )}
          <div className="border-t border-blue-300 pt-3 flex justify-between text-lg">
            <span className="font-bold">Tổng cộng:</span>
            <span className="font-bold text-blue-600">{costBreakdown.total.toLocaleString('vi-VN')}₫</span>
          </div>
        </div>
      </div>
    </div>
