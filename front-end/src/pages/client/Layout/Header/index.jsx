    ...
    try {
      ...
      if (token) {
        try {
          ...
          if (!response.ok) {
            console.warn("API logout không thành công");
          }
        } catch (apiError) {
          ...
          <div className="flex flex-wrap items-center justify-between py-2 sm:py-2.5 text-xs sm:text-sm gap-2">
            <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
              <a
                ...
                className="flex items-center gap-1.5 sm:gap-2 hover:text-blue-200 transition-colors duration-200"
              >
              ...
              <a
                ...
                className="hidden sm:flex items-center gap-1.5 sm:gap-2 hover:text-blue-200 transition-colors duration-200"
              >
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 bg-white/10 backdrop-blur-sm px-2 sm:px-3 py-1 sm:py-1.5 rounded-full">
              ...
              <span className="text-blue-100 hidden lg:inline text-xs">
                (2,500+ đánh giá)
              </span>
            </div>
          </div>
              ...
              {isLoggedIn ? (
                <div className="hidden lg:block relative">
                  ...
                  >
                    ...
                    />
                    <span className="hidden md:block text-sm lg:text-base text-gray-700 font-medium group-hover:text-blue-600 transition-colors duration-200 truncate">
                      {currentUser.name}
                    </span>
                    <FaAngleDown
                      className={`hidden md:block text-gray-500 text-sm transition-transform duration-300 ${
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4">
            <nav className="space-y-2 mb-4 border-b border-gray-200 pb-4">
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="block px-3 py-2 text-gray-700 font-medium hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-200"
                >
                  {item.label}
                </a>
              ))}
            ...
            {isLoggedIn ? (
              <div className="space-y-3">
                ...
                <Link
                  to="/orders"
                  className="flex items-center gap-3 px-3 py-2 text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-200"
                >
                  <FaBox className="text-blue-600" /> Đơn hàng của tôi
                </Link>
                ...
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-3 px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200 font-medium"
                >
              ...
              <div className="space-y-2">
                <Link
                  to="/dang-nhap"
                  className="block w-full px-4 py-2 text-center text-sm font-medium text-blue-600 border-2 border-blue-600 rounded-lg hover:bg-blue-50 transition-all duration-200"
                >
                  Đăng nhập
                </Link>
                <Link
                  to="/dang-ky"
                  className="block w-full px-4 py-2 text-center text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all duration-200"
                >
                  Đăng ký
                </Link>
              </div>
